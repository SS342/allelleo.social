...
class Status:
    def __init__(self, code, message, _type):
        self.code = code
...
class UsersDataBase(object):
    ...
    def new_user(self, nickname, email, name, surname, password, token):
        ...
        if not (UsersDataBase().get_user_by_email(email)):
            ...
            self.__connection__.commit()
            return {
                'status': Status(None, None, _type='Ok')
            }
        return {
            "status": Status(1, 'Данный e-mail уже ипользуется', _type='Error')
        }
